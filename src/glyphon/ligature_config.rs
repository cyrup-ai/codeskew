...
mod tests {
    ...
    fn test_default_config() {
        let config = LigatureConfig::default();
        assert!(config.enabled);
        assert!(config.font_overrides.is_empty());
    }

    #[test]
    fn test_font_override() {
        let mut config = LigatureConfig::enabled();
        config.set_font_override("Monaco".to_string(), false);

        assert!(config.is_enabled_for_font("FiraCode"));
        assert!(!config.is_enabled_for_font("Monaco"));
    }

    #[test]
    fn test_programming_defaults() {
        let config = LigatureConfig::with_programming_defaults();
        assert!(config.is_enabled_for_font("FiraCode"));
        assert!(!config.is_enabled_for_font("Monaco"));
    }

    #[test]
    fn test_font_ligature_detection() {
        assert!(LigatureConfig::font_has_good_ligature_support("FiraCode"));
        assert!(LigatureConfig::font_has_good_ligature_support(
            "JetBrains Mono"
        ));
        assert!(!LigatureConfig::font_has_good_ligature_support("Monaco"));
    }

    #[test]
    fn test_builder_pattern() {
        let config = LigatureConfigBuilder::new()
            .enabled(true)
            .font_override("TestFont".to_string(), false)
            .build();

        assert!(config.enabled);
        assert!(!config.is_enabled_for_font("TestFont"));
    }
    ...
    fn test_yaml_serialization() {
        ...
        let parsed = LigatureConfig::from_yaml(&yaml).unwrap();

        assert_eq!(config.enabled, parsed.enabled);
        assert_eq!(config.font_overrides.len(), parsed.font_overrides.len());
    }
}
