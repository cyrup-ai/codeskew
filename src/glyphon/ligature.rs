...
mod tests {
    ...
    #[test]
    fn test_ligature_helper_creation() {
        let helper = LigatureHelper::with_programming_defaults();
        assert!(helper.is_globally_enabled());
    }

    #[test]
    fn test_shaping_selection() {
        let helper = LigatureHelper::with_programming_defaults();

        // FiraCode should use Advanced shaping
        assert_eq!(helper.shaping_for_font("FiraCode"), Shaping::Advanced);

        // Monaco should use Basic shaping (limited ligature support)
        assert_eq!(helper.shaping_for_font("Monaco"), Shaping::Basic);
    }

    #[test]
    fn test_font_override() {
        let mut helper = LigatureHelper::enabled();
        helper.set_font_override("TestFont".to_string(), false);

        assert!(!helper.should_enable_ligatures("TestFont"));
        assert_eq!(helper.shaping_for_font("TestFont"), Shaping::Basic);
    }

    #[test]
    fn test_global_disable() {
        let mut helper = LigatureHelper::enabled();
        helper.set_globally_enabled(false);

        assert!(!helper.is_globally_enabled());
        assert_eq!(helper.shaping_for_font("FiraCode"), Shaping::Basic);
    }

    #[test]
    fn test_font_support_detection() {
        assert!(font_supports_ligatures("FiraCode"));
        assert!(font_supports_ligatures("JetBrains Mono"));
        assert!(!font_supports_ligatures("Monaco"));
        assert!(!font_supports_ligatures("Courier"));
    }

    #[test]
    fn test_recommended_shaping() {
        assert_eq!(recommended_shaping("FiraCode"), Shaping::Advanced);
        assert_eq!(recommended_shaping("Monaco"), Shaping::Basic);
    }
}
