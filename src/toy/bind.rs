...
/// Type alias for buffer serialization function to improve readability
type SerializeFn<H> = Box<dyn Fn(&H) -> Vec<u8>>;

pub struct BufferBinding<H> {
    pub host: H,
    //serialise: Box<dyn for<'a> Fn(&'a H) -> &'a [u8]>,
    serialise: SerializeFn<H>,
    device: wgpu::Buffer,
...
impl Bindings {
    pub fn new(wgpu: &WgpuContext, width: u32, height: u32, pass_f32: bool) -> Self {
        ...
        Bindings {
            ...
            storage1: BufferBinding {
                ...
                bind: Box::new(wgpu::Buffer::as_entire_buffer_binding),
                decl: "var<storage,read_write> buffer0: array<u32>".to_string(),
            },
            storage2: BufferBinding {
                ...
                layout: storage_buffer,
                bind: Box::new(wgpu::Buffer::as_entire_buffer_binding),
                decl: "var<storage,read_write> buffer1: array<u32>".to_string(),
            },
            ...
            font_atlas: {
                let font_texture = wgpu.device.create_texture(&wgpu::TextureDescriptor {
                ...
                TextureBinding {
                    view: font_texture.create_view(&Default::default()),
                    device: font_texture,
                    layout: wgpu::BindingType::Texture {
                        multisampled: false,
                        sample_type: wgpu::TextureSampleType::Float { filterable: true },
                    ...
                    decl: "var font_atlas: texture_2d<f32>".to_string(),
                }
            },
    ...
    /// Load font atlas texture data from the generated atlas
    pub fn load_font_atlas(&self, wgpu: &WgpuContext, atlas_data: &[u8]) {
        wgpu.queue.write_texture(
            wgpu::ImageCopyTexture {
                texture: &self.font_atlas.device,
                mip_level: 0,
            ...
            wgpu::ImageDataLayout {
                offset: 0,
                bytes_per_row: Some(1024), // R8Unorm format, 1 byte per pixel
